...
func moveToBackup() {
	...
	fmt.Println(today)
	sql := "insert into `pk_asia_pan_backup` select * from `pk_asia_pan` where schedule_bet_date < ?"
	ins_res, ins_err := myinit.Engine.Exec(sql, today)
	fmt.Println(ins_err, ins_res)
	del_sql := "delete from `pk_asia_pan` where schedule_bet_date < ?"
	...
	del_backup_sql := "delete from `pk_asia_pan_backup` where schedule_bet_date<?"
