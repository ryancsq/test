...
func ParseBetUrl(date string, history bool) {
	...
	for i, _ := range schedule_trs {
		...
		sql1 := "select predict1_result,count(*) as predict1_cnt from `pk_asia_pan` where schedule_date = ? and schedule_no=? group by predict1_result"
		...
		sql2 := "select predict2_result,count(*) as predict2_cnt from `pk_asia_pan` where schedule_date = ? and schedule_no=? group by predict2_result"
		...
		predict_json := make(map[string]string)
		predict_json["predict1"] = string(predict1_string)
		predict_json["predict2"] = string(predict2_string)
